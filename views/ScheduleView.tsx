...
const ScheduleView: React.FC = () => {
  ...
  return (
    <div className="p-4 space-y-4 max-w-2xl mx-auto relative">
        ...
        <div className="fixed inset-0 z-[100] bg-indigo-900/98 backdrop-blur-xl flex flex-col items-center justify-center p-10 text-center animate-in fade-in duration-300">
           <div className="bg-white/5 p-10 rounded-[3.5rem] border border-white/10 flex flex-col items-center space-y-8 w-full max-w-md shadow-2xl">
              ...
              <div className="space-y-3">
                 <h2 className="text-3xl font-black text-white tracking-tight">Redirecting...</h2>
                 <p className="text-indigo-200 text-sm font-bold leading-relaxed">
                   Opening official Pune Metro ticketing gateway.
                 </p>
              </div>
              <button 
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden flex flex-col">
              ...
              <div className="p-4 space-y-2.5 max-h-[480px] overflow-y-auto scrollbar-hide flex-1 bg-gradient-to-b from-white to-slate-50">
                {purpleStations.map((station, i) => (
                  <div key={i} className="flex items-center space-x-3 group">
                    <div className="flex flex-col items-center">
                      <Circle size={8} className="text-[#8b5cf6] fill-[#8b5cf6] group-hover:scale-125 transition-transform" />
                      {i < purpleStations.length - 1 && <div className="w-px h-5 bg-purple-100"></div>}
                    </div>
                    <div className="flex-1 pb-1">
                      <p className="text-[11px] font-black text-slate-800 group-hover:text-purple-600 transition-colors leading-tight">{station}</p>
                      <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Every 6 mins</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden flex flex-col">
              ...
              <div className="p-4 space-y-2.5 max-h-[480px] overflow-y-auto scrollbar-hide flex-1 bg-gradient-to-b from-white to-slate-50">
                {aquaStations.map((station, i) => (
                  <div key={i} className="flex items-center space-x-3 group">
                    <div className="flex flex-col items-center">
                      <Circle size={8} className="text-[#0ea5e9] fill-[#0ea5e9] group-hover:scale-125 transition-transform" />
                      {i < aquaStations.length - 1 && <div className="w-px h-5 bg-sky-100"></div>}
                    </div>
                    <div className="flex-1 pb-1">
                      <p className="text-[11px] font-black text-slate-800 group-hover:text-sky-600 transition-colors leading-tight">{station}</p>
                      <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Every 10 mins</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="px-1 pb-6">
            ...
            >
              ...
              <span className="tracking-widest">GENERATE A TICKET</span>
            </button>
          </div>
        </div>
      ) : (
      ...
      {selectedBus && (
        <div className="fixed inset-0 z-[60] bg-slate-50 overflow-y-auto animate-in slide-in-from-right duration-300">
          <div className="sticky top-0 bg-white border-b border-slate-100 p-5 flex items-center justify-between z-10 shadow-sm">
            <div className="flex items-center space-x-4">
              <button onClick={() => setSelectedBus(null)} className="p-3 bg-slate-50 hover:bg-slate-100 rounded-2xl text-slate-600 transition-all active:scale-90">
                <ChevronLeft size={24} />
              </button>
              <div>
                <h2 className="font-black text-slate-800 tracking-tight text-xl">Bus {selectedBus.route_number}</h2>
                <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">PMPML Official Schedule</p>
              </div>
            </div>
          ...
          <div className="p-6 space-y-8 pb-32">
             <div className="text-center space-y-4 pt-4">
                <div className="inline-flex items-center justify-center p-6 bg-white rounded-[2.5rem] shadow-xl border border-slate-100 mb-2">
                   <Bus size={48} className="text-emerald-600" />
                </div>
                <div className="space-y-1">
                   <div className="flex items-center justify-center space-x-4 text-slate-400 font-black uppercase text-[10px] tracking-[0.3em]">
                      <span>{selectedBus.origin}</span>
                      <ArrowRight size={14} />
                      <span>{selectedBus.destination}</span>
                   </div>
                   <h1 className="text-3xl font-black text-slate-900 tracking-tighter">{selectedBus.origin} ↔ {selectedBus.destination}</h1>
                </div>
             </div>

             <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm group">
                   <div className="bg-emerald-50 w-fit p-3 rounded-2xl text-emerald-600 mb-3 group-hover:bg-emerald-600 group-hover:text-white transition-all">
                      <Timer size={22} />
                   </div>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Frequency</p>
                   <p className="font-black text-slate-800 text-xl">{selectedBus.frequency}</p>
                </div>
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm group">
                   <div className="bg-indigo-50 w-fit p-3 rounded-2xl text-indigo-600 mb-3 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                      <MapPin size={22} />
                   </div>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Stops</p>
                   <p className="font-black text-slate-800 text-xl">{selectedBus.stopsCount} Stops</p>
                </div>
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm group">
                   <div className="bg-orange-50 w-fit p-3 rounded-2xl text-orange-600 mb-3 group-hover:bg-orange-600 group-hover:text-white transition-all">
                      <Clock size={22} />
                   </div>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">First Bus</p>
                   <p className="font-black text-slate-800 text-xl">{selectedBus.firstBus || '--'}</p>
                </div>
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm group">
                   <div className="bg-rose-50 w-fit p-3 rounded-2xl text-rose-600 mb-3 group-hover:bg-rose-600 group-hover:text-white transition-all">
                      <Clock size={22} />
                   </div>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Last Bus</p>
                   <p className="font-black text-slate-800 text-xl">{selectedBus.lastBus || '--'}</p>
                </div>
             </div>

             {selectedBus.travelTime && (
                <div className="bg-indigo-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden group">
                   <div className="relative z-10 flex items-center justify-between">
                      <div className="space-y-1">
                         <p className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.3em]">Estimated Travel Time</p>
                         <h3 className="text-4xl font-black tracking-tighter">{selectedBus.travelTime}</h3>
                      </div>
                      <div className="bg-white/10 p-5 rounded-3xl backdrop-blur-md group-hover:scale-110 transition-transform">
                         <Clock size={32} />
                      </div>
                   </div>
                   <div className="absolute -bottom-10 -right-10 w-48 h-48 bg-indigo-50 rounded-full opacity-20 blur-[80px]"></div>
                </div>
             )}
             ...
             <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center justify-between">
                <div className="flex items-center space-x-4">
                   ...
                   <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Starting Fare</p>
                      <p className="font-black text-slate-800 text-lg">₹{selectedBus.estimatedPrice || 10}</p>
                   </div>
                </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-slate-50 via-slate-50 to-transparent z-10">
            <button 
              onClick={() => window.open('https://pmpml.org/', '_blank')}
              className="w-full bg-slate-900 text-white py-6 rounded-[2.5rem] font-black text-lg shadow-2xl flex items-center justify-center space-x-4 active:scale-95 transition-all shadow-slate-900/30"
            >
